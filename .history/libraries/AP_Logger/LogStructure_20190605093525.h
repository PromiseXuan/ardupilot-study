...
struct PACKED log_Format_Multiplier {
    LOG_PACKET_HEADER;//日志头
    uint64_t time_us;
    char type;//类型
    double multiplier;//乘数，double
};
...
struct PACKED log_Format_Units {
    LOG_PACKET_HEADER;//日志头
    uint64_t time_us;
    uint8_t format_type;//格式类型
    char units[16];//unit 数组
    char multipliers[16];//multiplier数组
};

//Unit结构体，单位结构
struct UnitStructure {
    const char ID;//ID
    const char *unit;//unit单位 字符串
};

//结构Multiplier
struct MultiplierStructure {
    const char ID;//ID
    const double multiplier;//double乘数
};

// all units here should be base units 这里的所有单位都应该是基本单位
// This does mean battery capacity is here as "amp*second"  这意味着电池容量在这里为“安培 * 秒”
...
//可以和上面的UnitStructure结合看，log_Units是一个元素为UnitStructure的数组，{id,unit}
const struct UnitStructure log_Units[] = {
    ...
    { 'D', "deglatitude" },   // degrees of latitude纬度
