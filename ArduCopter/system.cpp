...
void Copter::init_ardupilot()
{
    ...
    serial_manager.init_console();//串口管理器初始化控制台，UARTA

    hal.console->printf("\n\nInit %s"
                        "\n\nFree RAM: %u\n",
                        AP::fwversion().fw_string,
                        (unsigned)hal.util->available_memory());
	//打印fireware固件的版本及可用内存空间大小
    //
    // Report firmware version code expect on console (check of actual EEPROM format version is done in load_parameters function)
    //
    report_version();//打印固件的版本编码
	//形式为第一行为版本编码号，第二行40个‘-’作为分隔符，第三行为2个空格符号。

    // load parameters from EEPROM
    load_parameters();//从EEPROM中加载参数，并完成了实际EEPROM版本检测

    // time per loop - this gets updated in the main loop() based on
    // actual loop rate
	//每个循环的时间：这将根据实际循环速率在主循环()中更新
    G_Dt = 1.0 / scheduler.get_loop_rate_hz();

#if STATS_ENABLED == ENABLED
    // initialise stats module  初始化统计模块
    g2.stats.init();    //ParametersG2 g2;g2依然是一个参数
#endif

    // identify ourselves correctly with the ground station
	//与地面站正确识别自己
    mavlink_system.sysid = g.sysid_this_mav;//地面站系统id获取
	...
	//更早地启动第一个串口去允许板初始化时报错
    gcs().chan(0).setup_uart(serial_manager, AP_SerialManager::SerialProtocol_MAVLink, 0);


    // Register mavlink_delay_cb, which will run anytime you have
    // more than 5ms remaining in your call to hal.scheduler->delay
	//注册mavlink_delay_cb，当你在hal.scheduler-> delay的调用中剩余超过5ms时，它将运行
    hal.scheduler->register_delay_callback(mavlink_delay_cb_static, 5);
    
    BoardConfig.init();//初始化板
#if HAL_WITH_UAVCAN
    ...
    attitude_control->parameter_sanity_check();//姿态控制:完整性检查参数。应该在起飞前调用一次
    pos_control->set_dt(scheduler.get_loop_period_s());//位置控制，为所有控制器设置以秒为单位的时间增量
    ...
    startup_INS_ground();//启动INS_ground
    ...
    // ready to fly
	//我们不希望写入串口导致我们在飞行途中暂停，因此一旦我们准备好飞行就将串口设置为非阻塞
    serial_manager.set_blocking_writes_all(false);//设置串口为false
    ...
    if (arming.rc_calibration_checks(true)){
        enable_motor_output();//如果武装rc校准检查是true，使能输出至电机
    }
    ...
    ap.initialised = true;//标记初始化已完成
}


//******************************************************************************
//This function does all the calibrations, etc. that we need during a ground start
//该功能可以完成在地面启动时所需的所有校准等
//******************************************************************************
