...
void AP_Scheduler::init(const AP_Scheduler::Task *tasks, uint8_t num_tasks, uint32_t log_performance_bit)
{
    ...
    _last_run = new uint16_t[_num_tasks];//new新分配一个动态数组，大小为传递进来的num_tasks,即该任务数组的大小
    memset(_last_run, 0, sizeof(_last_run[0]) * _num_tasks);//初始化_last_run，将其元素置为0
    _tick_counter = 0;
...
void AP_Scheduler::run(uint32_t time_available)
{
    ...
    uint32_t now = run_started_usec;//获取当前时间

    if (_debug > 1 && _perf_counters == nullptr) {
        _perf_counters = new AP_HAL::Util::perf_counter_t[_num_tasks];//new一个动态数组
    ...
    for (uint8_t i=0; i<_num_tasks; i++) {
        ...
        //_loop_rate_hz 是循环的频率，这里应该是50Hz，_tasks[i].rate_hz是本任务的频率，
        //假如是20Hz，那么interval_ticks =2.5，那就应该是运行了2圈后再进来运行任务时就该执行，即中间要间隔2.5圈
        if (interval_ticks < 1) {
            interval_ticks = 1;
        }
...
void AP_Scheduler::loop()
{
    ...
    const uint32_t sample_time_us = AP_HAL::micros();//����ʱ��
    
    if (_loop_timer_start_us == 0) {
        _loop_timer_start_us = sample_time_us;//���ѭ����ʱ��ʱ��Ϊ0�������䵱ǰ����ʱ��
        _last_loop_time_s = get_loop_period_s();//���һ��ѭ����ʱ��
    } else {
	...
	//��������Ҫ���е�������ע�⣬����ֻ��ÿ��ѭ������һ�Σ���Ϊ������ѭ��tick�ı������е��ȡ�
	//��ˣ��������û�����е�һ�ε��õ��ȳ������ǽ��������Ժ�ĵ��������У�ֱ���ٴε���scheduler.tick����Ϊֹ
    const uint32_t loop_us = get_loop_period_us();//��΢��Ϊ��λ��ȡÿ��ѭ��������ʱ��
    const uint32_t time_available = (sample_time_us + loop_us) - AP_HAL::micros();//�������ʱ�䣬
	
    run(time_available > loop_us ? 0u : time_available); //����һ��tick���⽫��ָ����ʱ�������о����ܶ�ĵ��ȳ�������
	//���time_available > loop_us��ִ��run(0u);���time_available <= loop_us,ִ��run(time_available);����run()����
