 ...
 */
//运行一个tick，这将在指定的时间内运行尽可能多的调度程序任务
...
void AP_Scheduler::loop()
{
    ...
    const uint32_t sample_time_us = AP_HAL::micros();//采样时间
    
    if (_loop_timer_start_us == 0) {
        _loop_timer_start_us = sample_time_us;//如果循环计时的时间为0，则赋予其当前采样时间
        _last_loop_time_s = get_loop_period_s();//最后一次循环的时间
    } else {
	...
	//运行所有要运行的任务。请注意，我们只需每个循环调用一次，因为任务按主循环tick的倍数进行调度。
	//因此，如果它们没有运行第一次调用调度程序，它们将不会在稍后的调用上运行，直到再次调用scheduler.tick（）为止
    const uint32_t loop_us = get_loop_period_us();//以微秒为单位获取每个循环允许的时间
    const uint32_t time_available = (sample_time_us + loop_us) - AP_HAL::micros();//计算可用时间，
	
    run(time_available > loop_us ? 0u : time_available); //运行一个tick，这将在指定的时间内运行尽可能多的调度程序任务
	//如果time_available > loop_us，执行run(0u);如果time_available <= loop_us,执行run(time_available);核心run()函数
