...
{
    serialmanager_p = &serial_manager;//获取串口管理器

    // search for serial port
    
    AP_HAL::UARTDriver *uart;
    uart = serial_manager.find_serial(protocol, instance);//查找串口
    if (uart == nullptr) {
    ...
    mavlink_channel_t mav_chan;    //mavlink通道
	//获取通道
    ...
    /*
      ...
      and boot normally
     */
    uart->begin(115200);//设置波特率115200
    AP_HAL::UARTDriver::flow_control old_flow_control = uart->get_flow_control();//获取流控制
    uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);//设置流控制为DISABLE
    for (uint8_t i=0; i<3; i++) {
        hal.scheduler->delay(1);//延时1
        uart->write(0x30);//发送0x30
        uart->write(0x20);//发送0x20
    }//在启动时发送0x30,0x20，告诉bootlader复位并且正常启动
